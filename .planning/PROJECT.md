...
A group of 1000 can jointly control a single Bitcoin address such that (a) any 501 can spend, (b) no individual ever holds the key, (c) membership can rotate without touching the chain or changing the address, and (d) past compromise is truly revocable by sweeping to a standby key. If everything else fails, the frost↔rust-bitcoin bridge must produce a valid BIP340 key-spend signature over the correct sighash.
...
- [ ] DKG at n=1000 over self-hosted relays: resumable, idempotent per `(ceremony_id, round, seat)`, relay rate-limit tuning, paced round-2 batches
...
- ECDSA of any kind — wrong signature type for Taproot key-spend and infeasible at n=1000
...
- **Security model:** untrusted individual participants (assumption: <501 of any single epoch's holders collude/leak); untrusted coordinator (can stall/censor, cannot forge/steal/alter address); relays trusted for liveness only, redundantly. Adversary is mobile (compromises devices over time; refresh resets progress via epoch-mixing) plus retained-share insiders (mitigated only by sweep).
- **Residual risk (normative):** 501 shares from *one* epoch reconstruct the key forever. Rotation defends against external gradual compromise only; the sweep is the real revocation. Value bounds the prize, churn bounds the coalition pool — both trigger sweeps, either alone insufficient.
