...
updated: 2026-07-14T13:15:32Z
---

## Current Test

number: 1
name: Interactive no-echo passphrase prompt on a fresh store
expected: |
  Running `cheget` on a fresh store at a real terminal (triggering
  InteractivePassphrase::for_new_store): the passphrase is NOT echoed as you
  type; you are prompted twice and a mismatch is rejected; the "a lost
  passphrase makes them unrecoverable — there is no reset" warning prints
  before the prompts.
