...
provides:
  ...
  - "small-n confirmed regtest key-spend + t=51/n=100 full-scale confirmed key-spend (runs by default) + adversarial gates"
...
key-decisions:
  ...
  - "n=100 run runs by default and is scale-overridable via CHEGET_SIGN_T/CHEGET_SIGN_N (default 51/100); passes in 8.93s post-speedup (re-verified 2026-07-15) — the multi-CPU-hour bottleneck noted in 01-02 was the original n=1000 target, since rescaled/optimized"
...
coverage:
  ...
  - id: D5
    ...
    verification:
      - kind: integration
        ref: "tests/inproc_sign.rs#inproc_sign_confirmed_regtest_key_spend_small_n (3-of-5 CONFIRMED, passing); tests/inproc_sign_100.rs#inproc_sign_confirmed_regtest_key_spend_51_of_100 (runs by default at t=51/n=100; CONFIRMED key-spend passes in 8.93s, re-verified 2026-07-15)"
        status: pass
    human_judgment: true
    rationale: "The small-n confirmed key-spend passes, and the identical full-scale 51/100 pipeline now also runs by default and passes (8.93s, re-verified 2026-07-15) after the post-completion DKG speedup — no nightly/human run needed. (The original ~multi-CPU-hour cost noted in 01-02 was the t=501/n=1000 target, since rescaled to 51/100.)"
...
- `tests/inproc_sign_100.rs` — the t=51/n=100 full-scale confirmed key-spend, runs by default (D-02/D-06), scale-overridable via `CHEGET_SIGN_T`/`CHEGET_SIGN_N`; passes in 8.93s (re-verified 2026-07-15).
...
- **n=100 run is default + scale-overridable:** the full-scale confirmed spend runs by default and passes in 8.93s (re-verified 2026-07-15). Its cost is dominated by the in-process DKG; at the original t=501/n=1000 target that was ~multi-CPU-hour (measured in 01-02), but the current 51/100 target completes in seconds after the speedup, so no nightly gate is needed.
...
- **Full 51/100 confirmed spend runs by default.** At the original t=501/n=1000 target the in-process DKG was the multi-CPU-hour bottleneck (as in 01-02), so this was initially a `#[ignore]` nightly gate. After the rescale to 51/100 (260713-jqs) and the DKG speedup (260713-itg), the full-scale confirmed key-spend was un-ignored (c537bf0) and now runs by default, passing in 8.93s (re-verified 2026-07-15) — no human/nightly run required.
...
- `cargo test --test inproc_sign` 7 passed (incl. CONFIRMED small-n regtest key-spend); `cargo test --test sign_adversarial` 3 passed; `cargo test --release --test inproc_sign_100` passes by default at 51/100 (1/1, 8.93s, re-verified 2026-07-15); `cargo test --test bridge_roundtrip` 3 passed (bridge refactor safe).
