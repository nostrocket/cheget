...
| **Estimated runtime** | quick ~30–60s · full ~2–5 min (regtest node spawn) · nightly n=1000 e2e separate |
...
- **Before `/gsd-verify-work`:** Full suite green; the `t=501/n=1000` nightly/on-demand e2e must pass before Phase 1 sign-off (D-06)
...
| 1-02-xx | 02 | 2 | KEY-01, KEY-02, KEY-05, (KEY-06) | T-1-dkg | 501-of-1000 in-process DKG yields one group key = internal key `P`; every seat confirms; mismatch aborts | unit + e2e | `cargo test dkg::` / nightly n=1000 | ❌ W0 | ⬜ pending |
...
| n=1000 O(n²) timing/memory measurement | KEY-06 (folded in) | The measurement *is* the deliverable — pass/fail is feasibility judgement, not a fixed threshold | Run nightly n=1000 e2e in `--release`; record per-part wall-clock + RSS |
