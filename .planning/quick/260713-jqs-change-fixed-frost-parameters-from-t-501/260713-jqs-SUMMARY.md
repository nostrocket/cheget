...
- **Task 4 — tests:** `git mv` renamed both full-scale files; crown-jewel fns renamed to `inproc_sign_confirmed_regtest_key_spend_51_of_100` and `dkg_100_all_shares_verify_to_one_group_key`; env defaults `CHEGET_SIGN_T`/`CHEGET_DKG_T`=51, `CHEGET_SIGN_N`/`CHEGET_DKG_N`=100. Recomputed `over_provisioned_poll_size` assertions: `(51,100)→57` (6-seat margin) and cap case `(51,55)→55`. Doc cross-references fixed in `inproc_sign.rs`, `dkg_small.rs`, `common/mod.rs`, `chain_backend_conformance.rs`, `regtest_fixture.rs`. `#[ignore]` strings corrected from the false "multi-CPU-hour / nightly" language to the real n=100 on-demand cost.
